...
>
			...
			<head>
				<title>
					<xsl:choose>
						...
						<xsl:otherwise>サイトマップインデックス</xsl:otherwise>
					</xsl:choose>
				</title>
		...
		.breadcrumb svg.back {
			...
			<body>
				...
				<xsl:call-template name="Header">
					<xsl:with-param name="title">サイトマップ</xsl:with-param>
	...
	<a href="https://girlslovin.com/"><span>家</span></a>
	...
	<a href="https://girlslovin.com/"><span>家</span></a>
			...
			<svg width="6" height="8" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M.727 7.06L3.78 4 .727.94l.94-.94 4 4-4 4-.94-.94z" fill="#141B38"/></svg>
					...
					<a href="https://girlslovin.com/sitemap.xml"><span>サイトマップインデックス</span></a>
						<svg width="6" height="8" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M.727 7.06L3.78 4 .727.94l.94-.94 4 4-4 4-.94-.94z" fill="#141B38"/></svg>
			...
			<p>
				<xsl:choose>
					<xsl:when test="$fileType='Sitemap' or $fileType='RSS'">
						このサイトマップには以下が含まれます:						<xsl:value-of select="$amountOfURLs"/>
						<xsl:choose>
					...
					<xsl:otherwise>
						このサイトマップインデックスには以下が含まれます:						<xsl:value-of select="$amountOfURLs"/>
						<xsl:choose>
							<xsl:when test="$amountOfURLs = 1">
								サイトマップ							</xsl:when>
							<xsl:otherwise>
								サイトマップ							</xsl:otherwise>
						</xsl:choose>
					</xsl:otherwise>
				</xsl:choose>
				2025年6月28日の10:50 PMに生成されました			</p>
		</xsl:if>
	</div>
</xsl:template>
	...
	<xsl:variable name="priorityLabel">
		<xsl:choose>
			<xsl:when test="$priorityLevel = 'low'">低</xsl:when>
			<xsl:when test="$priorityLevel = 'medium'">中</xsl:when>
			<xsl:when test="$priorityLevel = 'high'">高</xsl:when>
		</xsl:choose>
	</xsl:variable>
	...
	<a href="https://girlslovin.com/"><span>家</span></a>
		...
		<div class="empty-sitemap__buttons">
			<a href="https://girlslovin.com/" class="button">ホームページへ戻る</a>
					</div>
